...
$sql1 = "SELECT * FROM hoadon ORDER BY id LIMIT 1";
...
<body>
    ...
    <div class="hoadonct">
        ...
        <div class="ttin">
            ...
            <a style="text-decoration:none; float:right; margin-right:20px;" 
            onclick="return confirm('Bạn có chắc muốn hủy mã đơn DTMKN<?php echo htmlspecialchars($codegh); ?> không?')" 
            href="huydon.php?codegh=<?php echo htmlspecialchars($giohangData[0]['codegh']); ?>">Hủy đơn</a>
            <?php endif; ?>
            </p>
        </div>
        <div class="spham">
            ...
            $ship = 3.00;
             foreach ($giohangData as $giohangItem): 
                ...
                <div class="thongtin">
                    ...
                    <p style=" margin-left:15px; line-height:30px; margin-right:90px;"><?php echo htmlspecialchars($giohangItem['tensp']); ?><br>
                       $<?php echo number_format($giohangItem['gia'], 2); ?><br>
                       Số lượng: <?php echo htmlspecialchars($giohangItem['soluongsp']); ?></p>
                    <p style="line-height:30px; margin-left:100px;">$<?php echo number_format($tong, 2); ?></p>
                </div>
            <?php endforeach; ?>
